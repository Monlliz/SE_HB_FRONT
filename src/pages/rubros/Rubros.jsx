...
const GestionarRubros = () => {
  ...
  const cargarRubros = useCallback(async () => {
    ...
    try {
      ...
      const rubrosOrdenados = rubrosRaw.sort((a, b) => {
        const nombreA = a.nombre_rubro.toLowerCase();
        const nombreB = b.nombre_rubro.toLowerCase();

        // 1. Prioridad Máxima: "Examen" a la izquierda (inicio del array)
        const esExamenA = nombreA.includes("examen");
        const esExamenB = nombreB.includes("examen");

        if (esExamenA && !esExamenB) return 1; // A va antes
        if (!esExamenA && esExamenB) return -1; // B va antes

        // 2. Prioridad Mínima: "Trabajo Cotidiano" a la derecha (final del array)
        const esTCA = nombreA.includes("trabajo cotidiano");
        const esTCB = nombreB.includes("trabajo cotidiano");

        if (esTCA && !esTCB) return -1; // A va después (al final)
        if (!esTCA && esTCB) return 1; // B va después, así que A va antes

        // 3. El resto se queda como llegue (o puedes ordenar alfabéticamente si prefieres)
        return 0;
      });
  ...
  useEffect(() => {
    const cargarCalificaciones = async () => {
      ...
      try {
        ...
        const datosPTC = await fetchPromediosTc_service(
    ...
    return alumnos.map((alumno) => {
      ...
      rubros.forEach((rubro) => {
        ...
        if (esTC) {
          ...
          nota =
            valorMapa !== undefined && valorMapa !== null
              ? Number(valorMapa)
              : 0;
        }
    ...
    return datosTabla.map((alumno) => {
      ...
      rubros.forEach((rubro, index) => {
        ...
        if (rubro.nombre_rubro === "Trabajo Cotidiano") {
          row[headerKey] = alumno.promedioTc
            ? Number(alumno.promedioTc).toFixed(2)
            : "-";
        } else {
            ...
            <TableBody>
              {isEssentialLoading || loadingCalificaciones ? (
                ...
                datosFiltrados.map(
                  (
                    alumno,
                    index, // USAMOS datosFiltrados AQUÍ
                  ) => (
                    <TableRow
                    ...
                    >
                      ...
                      {rubros.map((rubro) => {
                        const esTrabajoCotidiano =
                          rubro.nombre_rubro === "Trabajo Cotidiano";
                        return (
                          <TableCell
                            key={`${alumno.alumno_matricula}-${rubro.id_rubro}`}
                            align="center"
                            sx={{
                              borderLeft: esTrabajoCotidiano
                                ? "3px solid #1976d2"
                                : "none",
                              bgcolor: esTrabajoCotidiano
